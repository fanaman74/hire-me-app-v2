      ...
      <form className="settings-layout" onSubmit={saveSettings}>
        <section className="settings-main panel">
          ...
          <div className="field-group">
            ...
            <small className="field-help">{keyStatus.source === 'environment' ? <><code>{provider.env}</code> is being used as a fallback because this account has no saved key. Saving a key here will use the account key for this profile.</> : provider.id === 'custom' ? <>This key is saved to your account profile and used only with the custom base URL above. Saved keys are masked and never returned.</> : <>This key is saved to your account profile. <code>{provider.env}</code> is used only when no account key is saved. Keys are masked and never returned.</>}</small>
          </div>
